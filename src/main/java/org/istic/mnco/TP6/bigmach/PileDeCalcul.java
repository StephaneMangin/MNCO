...
public static void empiler (int n) throws BigMachException {
	if (iPile >= taillePile - 1) {throw (new BigMachException (BigMachException.BM_DEBORDEMENT_HAUT)) ; }
	iPile++ ;
	tPile[iPile] = n ;
}

public static int dépiler () throws BigMachException {
	if (iPile < 0) {throw (new BigMachException (BigMachException.BM_DEBORDEMENT_BAS)) ; }
	iPile-- ;
	return tPile[iPile + 1] ;
}

public static void fixerSommet (int a) throws BigMachException {
	if (a < 0) {throw (new BigMachException (BigMachException.BM_DEBORDEMENT_BAS)) ; }
	if (a >= taillePile - 1) {throw (new BigMachException (BigMachException.BM_DEBORDEMENT_HAUT)) ; }
	iPile = a ;
}
