...
public class FaireAnalyseurSyntaxique {	
	...
	static boolean différé = false ;
	
	static String commentaire ;
	static String genAvant ;
	static String genAprès ;
	...
	public static void droite () {
		...
		if (s.equals("selection")) {aLaLigne("else if (");sélection();}
		consommer("corps");
		élément1 () ;
	...
	public static void droite1 () {
		consommer("pd") ;
		if (s.equals("selection")) {aLaLigne("if      (");sélection();}
		consommer("corps");
		élément1 () ;
		while (!s.equals("/corps")){
			élément () ;
	...
	public static void gauche () {
		if (s.charAt(0) >= 'A' && s.charAt(0) <= 'Z') {
			...
			// point de génération
			if (!pGVue) {genAvant = "Génération."+s.substring(1)+" () ;" ;}
			else {genAprès = "Génération."+s.substring(1)+" () ;" ;}
			commentaire = commentaire + " " + s ;
	...
	public static void règle () {
		...
		if (!genAprès.equals("")) {aLaLigne(genAprès); genAprès = "";}
		consommer("/regle") ;
	}
